document.addEventListener("DOMContentLoaded", async () => {
  ...
  blockedInput.addEventListener("input", () => {
    const words = blockedInput.value
      .split(",")
      .map((w) => w.trim())
      .filter(Boolean);
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    if (msg.action === "showSpam") {
      ...
      msg.comments.forEach((comment, i) => {
        ...
        div.innerHTML = `
            <input type="checkbox" id="spam-${i}" checked style="
    width: 20px;
    height: 20px;
    cursor: pointer;
    accent-color: #f8e323;
...
">
            <label for="spam-${i}" style="margin-left: 8px; user-select: none; cursor: pointer;">${comment.text}</label>
            `;
...
function reportCommentsById(commentIds) {
  async function reportCommentById(commentId) {
    const thread = Array.from(document.querySelectorAll("ytd-comment-thread-renderer")).find((t) => {
    ...
    const reportItem = Array.from(document.querySelectorAll("tp-yt-paper-item")).find((item) => item.innerText.toLowerCase().includes("report"));
    ...
    const spamOption = Array.from(document.querySelectorAll("label.radio-shape-wiz__label-container")).find((label) => label.innerText.toLowerCase().includes("spam") || label.innerText.toLowerCase().includes("misleading"));
    if (spamOption) {
      spamOption.click();
    } else {
    ...
    const confirmBtn = Array.from(document.querySelectorAll("button")).find((btn) => btn.innerText.trim().toLowerCase() === "report");
